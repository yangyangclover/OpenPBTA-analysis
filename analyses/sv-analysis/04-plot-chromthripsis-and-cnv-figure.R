...
list <- read.csv(file.path("scratch","sv-shatterseek","PBTA_chromothripsis_newlink_region.csv"))
# read file
...
independent_specimen_list <- read.table(file.path("data","independent-specimens.wgs.primary-plus.tsv"),header = TRUE,sep = "\t")
# bioid including all sample's names will be used later
bioid <- unique(independent_specimen_list$Kids_First_Biospecimen_ID)


## ===================== Load cnv file =====================
cnv_file <- read_tsv(file.path("analyses","copy_number_consensus_call","results","pbta-cnv-consensus.seg"))
...
cnv_analysis$id_chr <- paste(cnv_analysis$ID,cnv_analysis$chrom,sep = "_")
cnv_analysis <- as.data.frame(cnv_analysis)
# cnv_analysis_new <- cnv_analysis[,1:7]
# for (samplei in unique(cnv_analysis$id_chr)) {
#   cnvi <- cnv_analysis[cnv_analysis$id_chr == samplei,]
#   cnvi <-  cnvi[order(cnvi$loc.start),]
#   chri <- cnvi[,"chrom"][1]
#   
#   while (sum(diff(cnvi$loc.end) < 0) >0) {
#     cnvi  <- cnvi[-(which(diff(cnvi$loc.end)<0)+1),]
#   }
#   
#   locstarti <- cnvi[,"loc.start"]
#   locendi <- cnvi[,"loc.end"]
#   cnv_analysis_add <- data.frame(matrix(NA,ncol=7,nrow = length(locstarti)+1 ))
#   colnames(cnv_analysis_add) <-  colnames(cnvi)[1:7]
#   cnv_analysis_add$loc.start <- c(0,locendi+1) 
#   cnv_analysis_add$loc.end <- c(locstarti-1,as.numeric(seqlengths(Hsapiens)[paste0("chr",chri)])) 
#   cnv_analysis_add$ID <- cnvi[,"ID"][1]
#   cnv_analysis_add$chrom <- cnvi[,"chrom"][1]
#   if (chri != "X") {
#     cnv_analysis_add$copy.num <- 2
#   }
#   cnv_analysis_new <- rbind(cnv_analysis_new,cnv_analysis_add)
# }
# cnv_analysis_new2 <- cnv_analysis_new
# for (samplei in unique(cnv_analysis_new$ID)) {
#   cnvi <- cnv_analysis_new[cnv_analysis_new$ID == samplei,]
#   a = (seq(1,22,1) %in% cnv_analysis_new[cnv_analysis_new$ID == samplei,"chrom"])==FALSE
#   if (sum(a) != 0) {
#     cnv_analysis_add <- as.data.frame(matrix(NA,ncol=7,nrow=sum(a)))
#     colnames(cnv_analysis_add) <-  colnames(cnv_analysis)[1:7]
#     cnv_analysis_add$ID <- cnvi[,"ID"][1]
#     cnv_analysis_add$chrom <- seq(1,22,1)[a]
#     cnv_analysis_add$loc.start <- 0
#     cnv_analysis_add$loc.end <- as.numeric(seqlengths(Hsapiens)[paste0("chr",cnv_analysis_add$chrom)])
#     cnv_analysis_add$copy.num <- 2
#     cnv_analysis_new2 <- rbind(cnv_analysis_new2,cnv_analysis_add)
#   }
# }
...
for (j in 1:length(listuni$sample)){ 
  ...
  # read sv file
  sv.data <- read.delim(file.path("scratch","sv-vcf",paste(sample,".tsv",sep = "")))
  ...
  if (length(seq) < 3){
    for (i in 1:length(seq)) {
      ...
      cnv[cnv$chrom == seq[i],]$loc.end <- cnv[cnv$chrom == seq[i],]$loc.end + chrs_fake_starts[[i]] + 0
      region[region$chr==seq[i],]$start <- region[region$chr==seq[i],]$start + chrs_fake_starts[[i]]
      region[region$chr==seq[i],]$end <- region[region$chr==seq[i],]$end + chrs_fake_starts[[i]] + 0
    }
  } else {
    for (i in 1:length(seq)) {
      cnv[cnv$chrom==seq[i],]$loc.start <- cnv[cnv$chrom==seq[i],]$loc.start + chrs_fake_starts[[i]]
  ...
  names(beziers) <- c("x","y","group","svtype")
  ...
  # output figure
  jpegname <- paste0(listuni$cluster_id[j],"_consensus",".jpeg")
